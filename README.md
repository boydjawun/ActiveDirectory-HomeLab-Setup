...
This project demonstrates my expertise in system architecture, domain management, and building/managing complex network environments through a hands-on Active Directory (AD) Home Lab setup using VMware Virtualization Software. The following procedure involves configuring Microsoft Windows Server 2022 to host Active Directory Domain Services (AD DS) and deploying a Domain Controller (DC). A PowerShell script is utilized to automate the creation and addition of 1,000 users to AD, highlighting efficient administrative task automation. Following DC deployment, a Windows 11 Pro Virtual Machine is created as CLIENT1, configured with appropriate network settings, and joined to the domain. The next step is to log in to CLIENT1 using one of the script-generated domain user accounts, simulating a basic corporate network environment. The setup culminates as CLIENT1 is logged in using a created user to administer group policies and enforce delegation privileges for organizational units created in Active Directory. This project serves as a practical showcase for networking and AD administration skills.

# **Objectives**

- Download and install VMWare.
* Download Windows 11 ISO and Windows Server 2022 ISO to install the two operating systems on two separate virtual machines.
...
* Two network adapters
  ...
  * The other is used to connect to the DC’s private network that CLIENT1 is going to connect to.
...
* Set up DHCP on DC, so when CLIENT1’s Virtual Machine is created and added to the network, it can automatically get an IP address.
* Run a PowerShell script to create 1000+ users in the Active Directory automatically.

**Create the Second Virtual Machine**

* Install Windows 11 ISO.
* Connect Windows 11 Virtual Machine to the private DC’s network.
...
* Create and administer group policies to the Organizational Unit and verify that the policy works correctly
...
* Open the VMware application and click “Create a New Virtual Machine”.
...
* Leave the Disk Capacity as it is, 60.0 GB, click Next >, then click finish.
* My Virtual Machine only has one adapter; two are needed for the outside internet connection and a private internet connection for the clients.
...
- After partitioning the drive, allow the machine to restart, set an Administrator Password, and Log in with Administrator password.
> May Have to install VMware tools to combat lagging issues if its a problem
...
- I have to figure out which Network Interface Card(NIC) serves as a Network Access Translator(NAT) and name the two adapters appropriately.
    ...
    - Change Ethernet0 to "\_INTERNET\_" and Ethernet1 to "X_internal_X".
...
- After completing the installation process the Active Directory Domain Services software is downloaded, now the Domain needs to be created. On the Server Manager, click the flag at the top with the yellow triangle.
...
- In the Additional Options section, wait for the NetBIOS name to generate for the Domain and click Next > to the Prerequisites Check section, wait until the check is passed, and click Install. After the installation, the Virtual Machine restarted, and I logged back in with my Administrator account.
...
- Create and Organizational Unit for the Admin account that is being created.
>Think of an Organizational Unit as a folder used for the Admin account.
...
- I selected the “Password Never Expires” box to refrain from dealing with expiring passwords during this Lab then click Next >, and Finish.
...
- After creating the Domain Account, I signed out of the built-in Administrator account and signed in using the created Domain Admin Account.
...
- In the Server Manager, click “Add roles and features”. Select “Role-based or feature-based installation” for installation type.
...
- I clicked Next > until I got to the Confirmation section and clicked Install. Wait on the installation, then click Close.
...
- After clicking Finish, there was a pop up and I pressed OK.

![image.png](images/image%2035.png)

- Wait until the Remote Access Service is finished initializing.
...
Allows Windows 11 Clients to automatically get an IP address that will let them access the internet even though they are on the DC's private internal network. I will create a scope to give IP Addresses in a defined range.
...
- I chose my domain that I created (mydomain.local) as my parent domain and clicked Next > until I was asked about Activating the scope, and I selected “Yes, I want to activate this scope now”, then Next >, then Finish.
...
- First, I make a configuration that lets me browse the internet from the Domain Controller.
...
- Click the names.txt file in the folder. I added my name at the top of the names.txt file name list, and saved it.
...
- Before running this PowerShell script, I have to enable the execution of all scripts. Do this by typing: Set-ExecutionPolicy Unrestricted and press enter and select “Yes to All” for the Execution Policy Change pop up. This is for lab purposes only as this is a security feature for the Domain Controller.

![image.png](images/image%2057.png)

- To run, a path has to be defined to locate the PowerShell script. Change directory into the script file.
- After locating the file, click enter to run it and click “Run Once” at the Security Warning.

![image.png](images/image%2058.png)

- After the script has finished, I look on “Active Directory Users and Computers” and there are alot of users that were created from that PowerShell script.

![image.png](images/image%2059.png)

- The script ran fine, but there were a couple of errors due to duplicate names that were on the list. This did not effect the script completing.
...
- Click “Network Adapter”, then select Custom. This disables NAT and restricts internet access in my local network. CLIENT1 can access the internet, but only by getting an IP address from the Domain Controller. For this to happen, I configured this Virtual Machine on the same network as the Domain Controller Virtual Machine.
...
- After setup, I ran ipconfig in the terminal to see that the IP Addressing I set up with DHCP works, and I am able to access the internet through the Domain Controller.

![image.png](images/image%2071.png)

- I can also ping my Domain Controller which shows there is indeed network connectivity between the Windows 11(CLIENT1) and the Windows Server 2022(DC) Virtual Machines.

![image.png](images/image%2072.png)

- I changed the hostname of the Windows 11 system by right-clicking the start menu > System. Then click rename.

![image.png](images/image%2073.png)

- After renaming, I restarted the VM.

### Joining Windows 11 to Windows Server 2022 (Domain Controller)

- Click on the Window’s Icon and search “domain”, click on “Access work or school”, and click the “Connect” button.
...
- I used my Administrator Account that I previously made as credentials to join the domain, and for account type I chose Administrator. Then, I restarted my system and logged back in as user a-jboyd.
...
I am now logging in as another user account established within mydomain.local from running the PowerShell script to check for proper configuration .
...
The scenerio: I have a tech company, MyDomain, inc., and each department needs to have different rules and restrictions. This will be done using Organizational Units, container objects that let you classify users and machines.
...
- In the Server Manager on the DC, click tools > Active Directory Users and Computers.
...
- Click _USERS, and select a user to move. Right-click the user and select “Move…”
...
Now that I have the Organizational Units created, I want to delegate the control of resetting user passwords for every department to users in the IT Department.
...
- I added a user’s name and clicked “Check Names” to verify the user, then clicked space and typed the other names with the same procedure, and clicked Ok.
...
I log in as Carmello(cdally), from the IT Organizational Group, on CLIENT 1 and reset the password for Beverley(btagg) from the Marketing Group. To be able to interact with the Active Directory from CLIENT1, I had to download RSAT:Active Directory Domain Services and Lightweight Directory Service Tools.

>I had to be logged in as a local administrator for CLIENT1 to download RSAT. I logged in with my administrator account, downloaded it, and logged back in as cdally.
...
>Ensure that the account used has "Password never expires" unchecked in the user account properties in Active Directory. Tools > Active Directory Users and Computers > Double Click User > Account Tab
...
```Set-ADUser -ChangePasswordAtLogon $true -Identity btagg -Verbose```
...
```Set-ADUser -ChangePasswordAtLogon $true -Identity abirk -Verbose```
...
- I log out of Carmello’s account and to access Beverley’s, and I am prompted to enter a new password to enter. Further verifying that the delegation for cdally works fine.
...
- Navigate to the “Group Policy objects” folder in mydomain.local and Right-Click > New > Enter a name for the Group Policy. I chose “Restrict Control Panel Access”, and clicked OK
...
Now I log in as Beverley(btagg), a member of the Marketing Organizational Unit, and try to access the control panel on CLIENT1.
